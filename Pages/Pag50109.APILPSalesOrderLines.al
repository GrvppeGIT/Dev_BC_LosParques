...
page 50109 "APILP - Sales Order Lines"
{
    APIGroup = 'LPCustom';
    APIPublisher = 'LP';
    ApplicationArea = All;
    Caption = 'LPSalesOrderLines';
    DelayedInsert = true;
    PageType = API;
    APIVersion = 'v2.0';
    ...
    EntityName = 'LPsalesOrderLine';
    EntitySetName = 'LPsalesOrderLines';
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field(assemblyNo; Rec."APILP - Assembly Order No.")
                {
                    Caption = 'Assembly Order No.';
                    Editable = false;
                }
                field(assemblyId; Rec."APILP - Assembly Order Id")
                {
    ...
    var
        ...
        IdFilter: Text;
        FilterView: Text;
        AssemblyLink: record "Assemble-to-Order Link";
    ...
    begin
        ...
        IF Rec.FindFirst() then
            repeat
                SalesHeader.Reset();
                If SalesHeader.GetBySystemId(Rec."Document Id") then begin
                    Rec."APILP - Assembly Order No." := '';
                    Clear(Rec."APILP - Assembly Order Id");
                    AssemblyLink.Reset();
                    AssemblyLink.SetRange("Document Type", AssemblyLink."Document Type"::Order);
                    AssemblyLink.SetRange("Document No.", SalesHeader."No.");
                    AssemblyLink.SetRange("Document Line No.", Rec."Line No.");
                    if AssemblyLink.FindFirst() then begin
                        Rec."APILP - Assembly Order No." := AssemblyLink."Assembly Document No.";
                        AssemblyHeader.Reset();
                        AssemblyHeader.SetRange("Document Type", AssemblyLink."Assembly Document Type");
                        AssemblyHeader.SetRange("No.", AssemblyLink."Assembly Document No.");
                        if AssemblyHeader.FindFirst() then
                            Rec."APILP - Assembly Order Id" := AssemblyHeader.SystemId;
                        Rec.Modify(false);
                    end;
                end;
            Until Rec.Next() = 0;
