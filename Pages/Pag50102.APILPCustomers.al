...
page 50102 "APILP - Customers"
{
    APIGroup = 'LPCustom';
    APIPublisher = 'LP';
    APIVersion = 'v2.0';
    ApplicationArea = All;
    Caption = 'apilpCustomers';
    DelayedInsert = true;
    PageType = API;
    EntityCaption = 'Customer';
    EntitySetCaption = 'Customers';
    ChangeTrackingAllowed = true;
    EntityName = 'LPcustomer';
    EntitySetName = 'LPcustomers';
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field(defaultSalesPoint; Rec."FJH.Default Sales Point")
                {
                    Caption = 'Default Sales Point';
                    trigger OnValidate()
                    begin
                        RegisterFieldSet(Rec.FieldNo("FJH.Default Sales Point"));
                    end;
                }
                ...
                field(provinceCode; Rec."FJH.Province")
                {
                ...
                field(referenceOption802; Rec."FJH.Ref. Option 802 in XML")
                {
    ...
    begin
        ...
        if isSalesPointValue() then
            SalesPoint := Rec."FJH.Default Sales Point";
    ...
    local procedure isSalesPointValue() result: Boolean
    begin
        Result := False;
        TempFieldSet.Reset();
        TempFieldSet.SetRange(TableNo, Database::Customer);
        TempFieldSet.SetRange("No.", Rec.FieldNo("FJH.Default Sales Point"));
        if TempFieldSet.FindSet() then
            result := true;
        TempFieldSet.Reset();
        exit(Result);
